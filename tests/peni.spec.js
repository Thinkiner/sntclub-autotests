...
test('Пени: ставка пеней в уставе и претензии', async ({ page }) => {
  ...
  // ════════════════════════════════════════════════════════════
  // ШАГ 7: Выбрать собственника «№4, Тихий переулок (Клубничкин Глеб)»
  ...
  if (nativeSelectCount > 0) {
    ...
    for (let i = 0; i < nativeSelectCount; i++) {
      ...
      try {
        await sel.selectOption({ label: /Клубничкин Глеб/i });
  ...
  if (!ownerSelected) {
    ...
    ).filter({ hasText: /Клубничкин Глеб/i }).first();
  ...
  // ════════════════════════════════════════════════════════════
  // ШАГ 13: Выбрать «Да» в полях «Включить пени» и «Включить % пеней»
  // ════════════════════════════════════════════════════════════
  console.log('\n══════════════════════════════════════════');
  console.log('ШАГ 13: Выбираем "Да" в полях INCLUDE_PENI и INCLUDE_PERCENT_PENI');
  console.log('══════════════════════════════════════════');
  ...
  // ── INCLUDE_PERCENT_PENI: включить % пеней ──────────────────
  console.log('  → Выбираем "Да" в поле INCLUDE_PERCENT_PENI...');
  const includePercentSelect = page.locator('#INCLUDE_PERCENT_PENI');
  await expect(includePercentSelect).toBeVisible({ timeout: 10_000 });
  await includePercentSelect.selectOption('Y');
  const includePercentVal = await includePercentSelect.inputValue();
  expect(includePercentVal).toBe('Y');
  console.log('  ✓ INCLUDE_PERCENT_PENI = "Да"');
  ...
  const calcBtn = page.locator('#calc');
  ...
  // ── Нажимаем «Создать претензию» ─────────────────────────────
  // <button class="button ml-3" type="submit">Создать претензию</button>
  console.log('  → Нажимаем "Создать претензию"...');
  const createClaimSubmitBtn = page.locator('button[type="submit"]').filter({
    hasText: /Создать претензию/i,
  }).first();
  await expect(createClaimSubmitBtn).toBeVisible({ timeout: 10_000 });
  await createClaimSubmitBtn.scrollIntoViewIfNeeded();
  await createClaimSubmitBtn.click();
