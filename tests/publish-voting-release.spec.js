// ============================================================
// E2E тест: Публикация голосования на сайте release.sntclub.ru
// Тестовый фреймворк: Playwright Test (@playwright/test)
//
// ⚠️ Запускать вручную ПОСЛЕ успешного прохождения voting.spec.js
//    npx playwright test publish-voting-release.spec.js --headed
//
// Отличия от publish-voting.spec.js (prod):
//   1. Базовый URL — https://release.sntclub.ru/
...
// НАСТРОЙКИ ТЕСТА
// ─────────────────────────────────────────────────────────────
const BASE_URL = 'https://release.sntclub.ru';
...
// Basic Auth для доступа к площадке release (devsite:devsitesntclub)
...
// ОСНОВНОЙ ТЕСТ
// ─────────────────────────────────────────────────────────────
test('Публикация голосования [release]', async ({ page }) => {
  // Даём 180 секунд — release-стенд может работать медленнее
  test.setTimeout(180_000);

  // ════════════════════════════════════════════════════════════
  // Basic Auth: перехватываем все запросы и добавляем заголовок
  // Это нужно для доступа к площадке release, закрытой HTTP Basic Auth.
  // ════════════════════════════════════════════════════════════
  console.log('  → Настраиваем Basic Auth для release.sntclub.ru...');
