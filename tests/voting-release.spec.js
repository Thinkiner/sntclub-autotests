// ============================================================
// E2E тест: Голосование на сайте release.sntclub.ru
...
// ─────────────────────────────────────────────────────────────
// Учётные данные HTML-защиты тестовой площадки
// ─────────────────────────────────────────────────────────────
const BASIC_AUTH = {
  username: 'devsite',
  password: 'devsitesntclub',
};
...
async function login(page) {
  ...
  // ── HTML-защита (Basic Auth) ─────────────────────────────────
  // Тестовая площадка закрыта Basic Auth — передаём учётные данные
  // через page.authenticate() до первого goto(), чтобы браузер
  // автоматически отправлял заголовок Authorization при каждом запросе.
  console.log('  → Устанавливаем Basic Auth для HTML-защиты площадки...');
  await page.route('**/*', async (route) => {
    const headers = {
      ...route.request().headers(),
      Authorization: 'Basic ' + Buffer.from(`${BASIC_AUTH.username}:${BASIC_AUTH.password}`).toString('base64'),
    };
    await route.continue({ headers });
  });
  console.log('  ✓ Basic Auth установлен');

  // Страница авторизации расположена на корневом URL —
  // никакого редиректа на /login/ нет
  console.log('  → Открываем страницу авторизации https://release.sntclub.ru/ ...');
  await page.goto('https://release.sntclub.ru/', {
    waitUntil: 'domcontentloaded',
  });

  // Убеждаемся, что страница открылась
  await expect(page).toHaveURL(/release\.sntclub\.ru/);
  ...
  // У пользователя test.ramos@mail.ru несколько ролей — после входа
  // может появиться экран выбора ЛК: «Вам доступен вход в следующие ЛК:»
  // Нужно выбрать роль «Председатель» чтобы попасть в нужный ЛК.
  console.log('  → Проверяем, появился ли экран выбора роли...');
  await page.waitForTimeout(3_000); // даём время странице отреагировать на вход

  const roleSelectionText = page.getByText(/Вам доступен вход в следующие ЛК/i);
  const isRoleSelection = await roleSelectionText.isVisible();

  if (isRoleSelection) {
    console.log('  ℹ Найден экран выбора роли. Выбираем "Председатель"...');
    const predLink = page.getByRole('link', { name: /Председатель/i });
    await expect(predLink).toBeVisible({ timeout: 5_000 });
    await predLink.click();
    console.log('  ✓ Роль "Председатель" выбрана');
    await page.waitForLoadState('domcontentloaded');
  } else {
    console.log('  ✓ Экран выбора роли не появился — входим напрямую');
  }

  // Ждём появления элемента, который есть только у авторизованного
  // пользователя — ссылки «Главная» в боковом меню.
  // ⚠️ «Общие собрания и голосования» скрыт за кнопкой «БОЛЬШЕ» — не используем его здесь.
  console.log('  → Ожидаем появления бокового меню после входа...');
  await expect(
    page.getByRole('link', { name: /^Главная$/i })
...
test('Голосование: открыть форму организации собрания', async ({ page }) => {
  ...
  // ⚠️ Клик «Добавить пункт» вызывает addAgendaItemFunc() — функция может быть
  //    не определена на release, поэтому поля повестки (AGENDA_*) опциональны.
  console.log('  → Нажимаем "Добавить пункт Повестки дня" (первым!)...');
  const addAgendaBtn = page.locator('a.add_real_agenda_item').first();
  await expect(addAgendaBtn).toBeVisible({ timeout: 5_000 });
  await addAgendaBtn.scrollIntoViewIfNeeded();
  await addAgendaBtn.click();
  console.log('  ✓ Клик по a.add_real_agenda_item выполнен');

  // Ждём до 5 секунд для AJAX (на release addAgendaItemFunc может не быть определена)
  await page.waitForTimeout(3_000);

  // Ищем поле вопроса повестки (опционально — на release может не появиться)
  let agendaItemField;
  let foundFieldName = null;
  const agendaFieldSelectors = [
    '[name="AGENDA_0_question"]',
    '[name*="AGENDA"][name*="question"]',
    '[name*="question"]',
    '#main_agenda textarea',
    '#main_agenda input[type="text"]',
  ];
  for (const selector of agendaFieldSelectors) {
    const el = page.locator(selector).first();
    if (await el.isVisible()) {
      agendaItemField = el;
      foundFieldName = selector;
      break;
    }
  }
  if (agendaItemField) {
    console.log('  ✓ Поле повестки найдено:', foundFieldName);
  } else {
    console.log('  ⚠ Поля повестки не найдены — пропускаем заполнение агенды');
  }
  console.log('  ✓ ШАГ 5: добавление пункта повестки завершено');
  ...
  if (agendaItemField) {
    ...
    console.log('  ⚠ Поле повестки не найдено — пропускаем');
  }

  // ── 5. Описание рассмотрения (опционально на release) ─────────
  ...
  if (await descriptionField.isVisible()) {
    ...
    console.log('  ⚠ Поле AGENDA_0_description не найдено — пропускаем');
  }

  // ── 6–8. ЗА / ПРОТИВ / ВОЗДЕРЖАЛИСЬ (опционально) ────────────
  console.log('  → Заполняем поля голосования (ЗА / ПРОТИВ / ВОЗДЕРЖАЛИСЬ)...');
  const agreeField = page.locator('#AGENDA_0_agree');
  if (await agreeField.isVisible()) {
    await agreeField.fill('10');
  ...
  // ⚠️ На release AJAX добавления подпункта может не отработать — делаем опциональным
  ...
  if (await addSubItemBtn.first().isVisible()) {
    await page.waitForTimeout(500); // небольшая пауза перед кликом
    await addSubItemBtn.first().click();
    await page.waitForTimeout(3_000);
    if (await page.locator('[name="0_0_SubItem"]').isVisible()) {
      await page.locator('[name="0_0_SubItem"]').fill('Выбор председателя правления СНТ');
      console.log('  ✓ Подпункт к пункту №1 заполнен');
    } else {
      console.log('  ⚠ 0_0_SubItem не появился — пропускаем подпункт №1');
    }
  } else {
    console.log('  ⚠ Кнопка добавления подпункта №1 не найдена — пропускаем');
  }
  ...
  await page.waitForTimeout(1_000); // небольшая пауза перед кликом
  await addItemBtn.last().click();
  // Ждём до 15 сек — release может быть медленнее production
  const hasItem2 = await page.locator('[name="VOTEAGENDA_1_items"]').isVisible();
  if (!hasItem2) {
    await page.waitForTimeout(5_000);
  }
  if (await page.locator('[name="VOTEAGENDA_1_items"]').isVisible()) {
    ...
    console.log('  ⚠ VOTEAGENDA_1_items не появился — пропускаем пункт №2');
  }
  ...
  const subItemCount = await addSubItemBtn.count();
  if (subItemCount > 1) {
    await addSubItemBtn.last().click();
    await page.waitForTimeout(3_000);
    if (await page.locator('[name="1_0_SubItem"]').isVisible()) {
      await page.locator('[name="1_0_SubItem"]').fill('Утвердить смету в размере 500 000 руб.');
      console.log('  ✓ Подпункт к пункту №2 заполнен');
    } else {
      console.log('  ⚠ 1_0_SubItem не появился — пропускаем');
    }
  } else {
    console.log('  ⚠ Кнопка добавления подпункта №2 не найдена — пропускаем');
  }
